...
func (m *Manager) EnsureDirs() error {
	for _, comp := range []Component{ComponentHAClawOS, ComponentOpenClaw} {
...
func (m *Manager) GetStatus(comp Component) Status {
	...
	switch comp {
	case ComponentHAClawOS:
		st.ImageVersion = imageHAClawOSVersion()
...
func (m *Manager) GetAllStatus() AllStatus {
	return AllStatus{
		IsDocker:  IsDocker(),
		HAClawOS: m.GetStatus(ComponentHAClawOS),
