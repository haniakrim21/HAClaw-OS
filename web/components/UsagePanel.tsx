...
export const UsagePanel: React.FC<UsagePanelProps> = ({ sessionKey, gwReady, loadUsage, labels: a, session: s, onModelChange, loadModels }) => {
  ...
  // Clear stale error when gateway disconnects
  useEffect(() => {
    if (!gwReady) { setError(null); setUsage(null); }
  }, [gwReady]);
      ...
      <div className="p-3 space-y-3">
        ...
        {!gwReady && (
          <div className="flex flex-col items-center gap-1.5 py-3 text-center">
            <span className="material-symbols-outlined text-[14px] text-slate-300 dark:text-white/20">cloud_off</span>
            <p className="text-[10px] text-slate-400 dark:text-white/25">{a.notConnected || 'Gateway not connected'}</p>
          </div>
        )}

        {/* Error state (only shown when gateway IS ready) */}
        {gwReady && error && (
