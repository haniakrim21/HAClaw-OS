...
const SnapshotTab: React.FC<SnapshotTabProps> = ({ s, inputCls, labelCls, rowCls }) => {
  ...
  const [backupMethod, setBackupMethod] = useState<'haclawx' | 'openclaw'>('haclawx');
        ...
        {snapshotModeTab === 'manual' && (<div className={rowCls}><div className="px-4 py-3 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 p-0.5 rounded-lg bg-slate-100 dark:bg-white/[0.05]">
              <button type="button" onClick={() => setBackupMethod('haclawx')} className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${backupMethod === 'haclawx' ? 'bg-white dark:bg-white/10 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-white/60 hover:text-slate-700 dark:hover:text-white/80'}`}>HAClaw-OS</button>
